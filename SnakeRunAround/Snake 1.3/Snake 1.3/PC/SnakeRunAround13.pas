...
var  ran:conran;                           {Khai bao cac player}
     ran2:conran;
...
begin
textcolor(white);
gotoxy(dchx,dchy);write(chr(247));//gotoxy(32,20);
gotoxy(1,1);
dch:=readkey;
case dch of
  's':begin  gotoxy(dchx,dchy);write('  ');dchy:=dchy+1;chedo:=chedo+1; end;
  'w':begin  gotoxy(dchx,dchy);write('  ');dchy:=dchy-1;chedo:=chedo-1; end;
end;
...
// Doc eDo(xch,ych,camthanh,3,10,12);
...
begin
 if (rannao.thanran[1].x=item[2].x)and(rannao.thanran[1].y=item[2].y) then
  begin
     ...
     begin
        if (ran=1) then item1[13].show:=true
        else if (ran=2) then item1[13].an:=true;
        if (dch=KeyP1.up) then dch:=KeyP2.up
        else if (dch=KeyP1.down) then dch:=KeyP2.down
        else if (dch=KeyP1.right) then dch:=KeyP2.right
        else if (dch=KeyP1.left) then dch:=KeyP2.left;
        if (dch2=KeyP2.up) then dch2:=KeyP1.up
        else if (dch2=KeyP2.down) then dch2:=KeyP1.down
        else if (dch2=KeyP2.right) then dch2:=KeyP1.right
        else if (dch2=KeyP2.left) then dch2:=KeyP1.left;
         buffer:=KeyP1.up;KeyP1.up:=KeyP2.up;KeyP2.up:=buffer;
         buffer:=KeyP1.down;KeyP1.down:=KeyP2.down;KeyP2.down:=buffer;
         buffer:=KeyP1.left;KeyP1.left:=KeyP2.left;KeyP2.left:=buffer;
         buffer:=KeyP1.right;KeyP1.right:=KeyP2.right;KeyP2.right:=buffer;

         item1[13].time:=100;
         textcolor(lightred);
         gotoxy(67,14);write('Item ~player');delay(800);
         gotoxy(67,14);write('            ');
         gotoxy(67,14);write('T.gian: ',item1[13].time);delay(100);
     end else rannao.diem:=rannao.diem+1;
...
gotoxy(37,16);write('RUN AROUND');delay(500);textcolor(lightgreen);gotoxy(48,16);write('1.3 BETA');
delay(1400); *)
batdau: repeat
clrscr;
amthanh;
...
while (aba = 1) do
  ...
  { readln;}
   ...
   else if (dch=KeyP1.right)and(dchcu=KeyP1.left) then dch:=dchcu
   else if (dch=KeyP1.up)and(dchcu=KeyP1.down) then dch:=dchcu
   else if (dch=KeyP1.down)and(dchcu=KeyP1.up) then dch:=dchcu;
   if multiplayer<>1 then
   begin
    ...
    else if (dch2=KeyP2.right)and(dchcu2=KeyP2.left) then dch2:=dchcu2
    else if (dch2=KeyP2.up)and(dchcu2=KeyP2.down) then dch2:=dchcu2
    else if (dch2=KeyP2.down)and(dchcu2=KeyP2.up)then dch2:=dchcu2


    end;
        if (dch=KeyP1.left) then x:=x-1
        else if (dch=KeyP1.right) then x:=x+1
        else if (dch=KeyP1.up) then y:=y-1
        else if (dch=KeyP1.down) then y:=y+1
        else dch:=dchcu;
     if (ran2.thanran[1].x<>0) then begin
        if (dch2=KeyP2.left)or(dch=#74) then x3:=x3-1
        else if (dch2=KeyP2.right)or(dch=#76) then x3:=x3+1
        else if (dch2=KeyP2.up)or(dch=#73) then y3:=y3-1
        else if (dch2=KeyP2.down)or(dch=#75) then y3:=y3+1;
        end else dch2:=#0;

    end;
    if multiplayer=1 then
    begin
       { if (dch=KeyP1.left)and(item1[11].show=true) then x:=x+1 else} if (dch=KeyP1.left) then x:=x-1
       { if (dch=KeyP1.right)and(item1[11].show=true) then x:=x-1 else} else if (dch=KeyP1.right) then x:=x+1
       { if (dch=KeyP1.up)and(item1[11].show=true) then y:=y+1 else} else if (dch=KeyP1.up) then y:=y-1
       { if (dch=KeyP1.down)and(item1[11].show=true) then y:=y-1 else} else if (dch=KeyP1.down) then y:=y+1;
    end;
   ...
   begin
   kiemtraanitem(3);kiemtraanitem(4);kiemtraanitem(5);kiemtraanitem(11);kiemtraanitem(13);
  ...
  { readln;}
   ...
   if (x=x2)and(y=y2) then
      ...
      for i:=1 to soluongbomb do
       if (ran.thanran[j].x=bomb[i].x)and(ran.thanran[j].y=bomb[i].y) then
        begin
         ...
         if (multiplayer=3) then
          begin
           gotoxy(30,12);write('Player 2 win');
           gotoxy(30,11);write('Player 1 die');
           readln; goto batdau;
          end
         else begin
          lost;goto batdau;
      ...
      for i:=1 to soluongbomb do
       if (ran2.thanran[j].x=bomb[i].x)and(ran2.thanran[j].y=bomb[i].y) then
        begin
         ...
         if (multiplayer=3) then
          begin
           gotoxy(30,12);write('Player 1 win');
           gotoxy(30,11);write('Player 2 die');
           readln; goto batdau;
          end
         else begin
          lost;goto batdau;
    ...
    {if multiplayer=3 then
     ...
     {if multiplayer=3 then
     for i:=1 to ran2.dairan do
       if (ran.thanran[1].x=ran2.thanran[i].x)and(ran.thanran[1].y=ran2.thanran[i].y) then
         begin
         textcolor(white);
          gotoxy(30,13);write('Player 2 win');
          readln;
          exit;
         end; }

    {Kiem tra 2 con ran dam nhau trong battle}
    if multiplayer=3 then
    begin
     for i:=1 to ran.dairan do
